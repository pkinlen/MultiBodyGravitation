...
// This class will instantiate the BodiesMoving class which does the main 
// work evaluating the motion of bodies under gravity.
// FindPath:findPositionsForFlyBy() will attempt to find starting position of the 
// planets to get close flybys for the meteorite. 
// On the other hand, the method:
// calculateMovementOfBodies() will just evaluate the paths taken by the 
// bodies given a single initial position.
...
public class FindPath {
    ...
    public Config  getConfigData(boolean withOutput){
        ...
        double  flyByTargetDistance = 2e8;      // the closest distance the meteorite gets to the centre of the objects
    ...
    public ArrayList<Body> getBodiesDataPolarCoords(){
        ...
        bodies.add(new Body("Meteorite", 3e9,        3e2,          8e9,  0.0,  0.0,          -2e6, 1e6, 0.0   )); 
    ...
    public static void main(String[] args) {
         ...
         //fp.findPositionsForFlyBy();   // this attempts to find starting positions to obtain flybys.
    ...
    NumericalSolver  getSolver(final int planet, final double[] thetas, final Config config){
          ...
          double startTheta0     = thetas[planet];
          double startTheta1     = thetas[planet] + 0.01;
          double tol             = 1e-4;
          int    maxSteps        = 40;
          
          NumericalSolver solver = new NumericalSolver(flyByTargetDist, startTheta0, startTheta1, 
...
/* Possible extensions and improvements:
  bodies could spin
  could check for collisions, when have big steps
